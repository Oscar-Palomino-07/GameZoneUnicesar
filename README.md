...
- `warranties.csv` — warranties generated by console sales, one row per warranty with a `BASIC`/`EXTENDED` type discriminator (they only store the sale and product identifiers; the end date is calculated from the start date and the warranty type)
...
- `warranty-analysis.md` — warranty design orienting questions (Q1–Q5)
- `warranty-class-diagram.md` — warranty class diagram with the sales integration (Mermaid)
